...
class FaerunBot(discord.Client):
    ...
    async def setup_hook(self):
        ...
        @self.tree.command(name="faerun_complet",
        ...
        # /faerun_jdr : Convertir une date
        @self.tree.command(name="faerun_jdr",
                           description="Convertit une date (JJ-MM-AAAA) en date Faerûnienne")
        @app_commands.describe(date_str="Date au format JJ-MM-AAAA")
        async def faerun_jdr_command(interaction: discord.Interaction, date_str: str):
            try:
        ...
        # /mention_someone : Mentions d'une personne
        @self.tree.command(name="mention_someone",
        ...
        async def mentions_command(interaction: discord.Interaction, membre: discord.Member = None):
        ...
        # /mention_list : Mentions de tous
        @self.tree.command(name="mention_list",
                           description="Compte les mentions de tous les posteurs de ce salon dans #recompenses (30 derniers jours)")
        ...
        # /recap_mj : Messages multi-mentions
        @self.tree.command(name="recap_mj",
                           description="Compte le nombre de messages dans #recompenses sur 30j où le membre mentionne au moins deux personnes")
        @app_commands.describe(membre="Le membre dont on veut la stat (par défaut toi)")
        async def recap_mj_command(interaction: discord.Interaction, membre: discord.Member = None):
            cible = membre or interaction.user
            channel = discord.utils.get(interaction.guild.text_channels, name='recompenses')
            if not channel:
                await interaction.response.send_message("❌ Le canal #recompenses est introuvable.", ephemeral=True)
        ...
        # /mes_quêtes : Quêtes mentionnées
        @self.tree.command(name="mes_quêtes",
        ...
        async def mes_quetes_command(interaction: discord.Interaction, membre: discord.Member = None):
            cible = membre or interaction.user
