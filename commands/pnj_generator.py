# commands/pnj_generator.py - PARTIE 1/5
import discord
...
• **Dévotion :** {details.get('info_extra', 'Serviteur fidèle')}"""
    ...
    async def _send_long_content(self, interaction: discord.Interaction, content: str, embed: discord.Embed):
        ...
        if len(content) > 1900:
            ...
            await interaction.followup.send(f"```\n{content}\n```")
=======
        # commands/pnj_generator.py - PARTIE 2/5
# (Suite de la classe PnjGeneratorCommand)
...
=======
    ...
    *PNJ généré automatiquement - Prêt à utiliser*"""
            ...
            return template.strip()
        # commands/pnj_generator.py - PARTIE 3/5
# (Suite des méthodes de formatage)
    ...
    • **Expérience :** {details.get('info_extra', 'Étudie la magie')}"""
                ...
                return "• **Détails :** À développer selon les besoins de la campagne"
    # commands/pnj_generator.py - PARTIE 4/5
# (Suite des méthodes utilitaires)
...
=======
        ...
        def _generate_appearance(self, race: str, genre: str) -> str:
            ...
            # commands/pnj_generator.py - PARTIE 5/5 (FINALE)
# (Suite et fin de la classe PnjGeneratorCommand)
...
=======
                "richesse": random.choice(["prospère", "modeste", "en difficulté", "très riche"]),
                "reputation": random.choice(["honnête", "de confiance", "un peu louche", "réputé pour ses prix"]),
                "info_extra": "Tient boutique depuis " + str(random.randint(5, 30)) + " ans"
            },
            "noble": {
                "titre": random.choice(["Lord/Lady", "Baron/Baronne", "Comte/Comtesse", "Duc/Duchesse"]),
                "domaine": random.choice(["terres agricoles", "mines d'or", "vignobles", "forêts", "ports commerciaux"]),
                "influence": random.choice(["locale", "régionale", "courtisane", "militaire"]),
                "info_extra": "Famille noble depuis " + str(random.randint(3, 12)) + " générations"
            },
            "garde": {
                "rang": random.choice(["simple garde", "sergent", "capitaine", "lieutenant"]),
                "experience": str(random.randint(2, 20)) + " ans de service",
                "specialite": random.choice(["patrouilles", "enquêtes", "formation", "garde du corps"]),
                "info_extra": random.choice(["ancien soldat", "natif de la ville", "recruté récemment"])
            },
            "aubergiste": {
                "etablissement": random.choice(["auberge de luxe", "taverne populaire", "relais de voyageurs", "gîte familial"]),
                "reputation": random.choice(["accueillant", "discret", "convivial", "respecté"]),
                "specialite_culinaire": random.choice(["ragoûts copieux", "pâtisseries", "bières locales", "cuisine régionale"]),
                "info_extra": "Gérant depuis " + str(random.randint(3, 25)) + " ans"
            },
            "pretre": {
                "divinite": random.choice(["Tyr", "Helm", "Lathander", "Selûne", "Tempus", "Mystra", "Oghma", "Torm"]),
                "rang_clerical": random.choice(["acolyte", "prêtre", "grand prêtre", "chapelain"]),
                "temple": random.choice(["temple principal", "chapelle locale", "sanctuaire", "monastère"]),
                "info_extra": "Servant fidèle depuis " + str(random.randint(5, 30)) + " ans"
            },
            "aventurier": {
                "classe": random.choice(["guerrier", "rôdeur", "roublard", "mage", "clerc"]),
                "niveau_estime": random.choice(["débutant", "expérimenté", "vétéran", "légendaire"]),
                "specialite": random.choice(["exploration de donjons", "chasse aux monstres", "escorte de caravanes", "missions diplomatiques"]),
                "info_extra": "Aventurier depuis " + str(random.randint(1, 15)) + " ans"
            },
            "artisan": {
                "metier": random.choice(["forgeron", "menuisier", "tisserand", "potier", "bijoutier", "cordonnier", "tailleur", "boulanger"]),
                "reputation": random.choice(["maître artisan", "apprenti talentueux", "créateur renommé", "artisan respecté"]),
                "specialite": random.choice(["travail de précision", "créations uniques", "réparations rapides", "commandes spéciales"]),
                "info_extra": "Pratique son art depuis " + str(random.randint(3, 25)) + " ans"
            },
            "paysan": {
                "activite": random.choice(["agriculture", "élevage", "viticulture", "apiculture", "maraîchage", "sylviculture"]),
                "statut": random.choice(["propriétaire", "fermier", "métayer", "ouvrier agricole"]),
                "specialite": random.choice(["cultures céréalières", "légumes", "fruits", "bétail", "produits laitiers", "miel et cire"]),
                "info_extra": "Travaille la terre depuis " + str(random.randint(5, 40)) + " ans"
            },
            "voleur": {
                "specialite": random.choice(["pickpocket", "cambrioleur", "escroc", "contrebandier", "receleur", "assassin"]),
                "reputation": random.choice(["discret et efficace", "audacieux", "dangereux", "mystérieux", "recherché"]),
                "territoire": random.choice(["quartiers riches", "docks", "marché", "routes commerciales", "tavernes", "arrière-cours"]),
                "info_extra": "Actif dans le milieu depuis " + str(random.randint(2, 20)) + " ans"
            },
            "mage": {
                "ecole_magie": random.choice(["évocation", "illusion", "enchantement", "divination", "nécromancie", "transmutation", "abjuration", "invocation"]),
                "niveau_estime": random.choice(["apprenti", "magicien confirmé", "maître", "archimage"]),
                "specialite": random.choice(["recherche théorique", "création d'objets magiques", "sorts de combat", "sorts utilitaires", "rituels anciens"]),
>>>>>>> Stashed changes
...
=======
        """Crée un embed Discord pour afficher le PNJ (méthode de compatibilité)"""
>>>>>>> Stashed changes
