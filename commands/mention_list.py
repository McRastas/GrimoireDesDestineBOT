...
class FaerunBot(discord.Client):
    ...
    async def add_commands(self):
        ...
        # /help
        @self.tree.command(name="help", description="Affiche l'aide du bot")
        async def help_command(interaction: discord.Interaction):
            # Vérifier si l'utilisateur est admin
            is_admin = interaction.user.guild_permissions.administrator
            
            embed = discord.Embed(
                title="📚 Aide du Bot Faerûn",
            ...
            # Commandes Faerûn
            faerun_commands = """
            `/faerun` - Affiche la date actuelle en calendrier de Faerûn
            `/faerunfestival` - Montre le prochain festival de Faerûn
            `/faeruncomplet` - Affiche toutes les infos (date, saison, semaine)
            `/faerunjdr` - Convertit une date réelle en date Faerûnienne
            """
            embed.add_field(name="📅 Commandes Faerûn", value=faerun_commands, inline=False)
            
            # Commandes de mentions
            mentions_commands = """
            `/mentionsomeone` - Compte les mentions d'un membre dans #recompenses
            `/mentionlist` - Liste les mentions de tous les posteurs du salon actuel
            `/recapmj` - Compte les messages où un MJ mentionne 2+ personnes
            """
            embed.add_field(name="📢 Commandes Mentions", value=mentions_commands, inline=False)
            
            # Commandes utilitaires
            utils_commands = """
            `/mesquetes` - Affiche ton calendrier de quêtes à venir
            `/test` - Vérifie que le bot fonctionne
            `/info` - Informations sur le bot
            `/help` - Affiche cette aide
            """
            embed.add_field(name="🛠️ Commandes Utilitaires", value=utils_commands, inline=False)
            
            # Section admin si l'utilisateur est admin
            if is_admin:
                embed.add_field(name="━━━━━━━━━━━━━━━", value="**COMMANDES ADMIN**", inline=False)
                
                admin_commands = """
                `!sync_bot` - Synchronise les commandes slash avec Discord
                `!debug_bot` - Affiche les informations de debug
                `!purge_commands` - Supprime et resynchronise toutes les commandes
                `!clear_commands` - Supprime uniquement les commandes du serveur
                """
                embed.add_field(
                    name="🔧 Commandes Admin", 
                    value=admin_commands + "\n\n⚠️ *Ces commandes sont temporaires (10-15s)*", 
                    inline=False
                )
                
                # Conseils pour admin
                admin_tips = """
                • Utilisez `!sync_bot` après avoir invité le bot
                • Utilisez `!purge_commands` pour nettoyer les anciennes commandes
                • Les messages des commandes admin disparaissent automatiquement
                """
                embed.add_field(name="💡 Conseils Admin", value=admin_tips, inline=False)
            
            # Footer
            embed.set_footer(
                text="Bot Faerûn • Calendrier de Harptos" + (" • Mode Admin" if is_admin else ""),
                icon_url=interaction.guild.icon.url if interaction.guild.icon else None
            )
            
            await interaction.response.send_message(embed=embed, ephemeral=True) Informations du Bot",
                                  description="Bot Faerûn - Calendrier D&D",
                                  color=0x00ff00)
            embed.add_field(name="Guildes",
                            value=len(self.guilds),
        ...
        @self.tree.command(
            name="mentionlist",
            description=
            "Compte les mentions et posts de récompenses des posteurs du canal (30 derniers jours)"
        )
                        ...
                        try:
                            ...
                            if len(date_match.groups()) >= 3 and date_match.group(3):
                                ...
                                # Si on est en début d'année (janvier/février) et que la date est en fin d'année
                                    ...
                                    try:
                                        test_date = datetime(annee, mois, jour, tzinfo=timezone.utc)
                                        if test_date < now:
                            ...
                            quest_date = datetime(annee, mois, jour, tzinfo=timezone.utc)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.content.strip() == "!sync_bot":
            logger.info(f"Commande !sync_bot par {message.author.name} ({message.author.id})")
            ...
            if not message.author.guild_permissions.administrator:
                msg = await message.channel.send("❌ Permission refusée - Administrateur requis")
                await asyncio.sleep(5)
                await msg.delete()
                return

            status_msg = await message.channel.send("🔄 Synchronisation en cours...")
            ...
            try:
                ...
                if synced:
                    ...
                    embed.add_field(name="Commandes", value=cmd_list, inline=False)

                embed.add_field(name="Serveur", value=message.guild.name, inline=True)
                embed.add_field(name="ID Serveur", value=str(message.guild.id), inline=True)
        ...
        # Commande de debug
            ...
            embed.add_field(name="Commandes dans tree", value=len(self.tree.get_commands()), inline=True)
            embed.add_field(name="Guildes", value=len(self.guilds), inline=True)
            embed.add_field(name="Synced", value=self.synced, inline=True)

            # Lister les commandes
            cmd_list = "\n".join([f"• {cmd.name}" for cmd in self.tree.get_commands()])
            if cmd_list:
                embed.add_field(name="Liste des commandes", value=cmd_list, inline=False)

            # Vérifier les commandes de guild
            try:
                guild_commands = await self.tree.fetch_commands(guild=message.guild)
                embed.add_field(name="Commandes sur le serveur", value=len(guild_commands), inline=True)
            except:
                pass
        ...
        # Commande clear simple
            ...
            try:
                ...
                if commands:
                    for cmd in commands:
                        await cmd.delete()
                    msg = await message.channel.send(f"✅ {len(commands)} commande(s) supprimée(s) de ce serveur")
                else:
                    msg = await message.channel.send("❌ Aucune commande trouvée sur ce serveur")
